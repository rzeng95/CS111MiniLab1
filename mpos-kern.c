...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (current->next) {
			pid_t p_next = current->next;
			proc_array[p_next].p_state = P_RUNNABLE; 

		} 
		//exit_process(current->p_pid); //call the helper function defined below
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			proc_array[p].p_state = P_EMPTY; //"reap" a zombie process

		}
		else {
			//current->p_registers.reg_eax = WAIT_TRYAGAIN;
			current->p_state = P_BLOCKED;
			proc_array[p].next = current->p_pid;
		}
		schedule();
	}
...
static void exit_process(pid_t toKill) {
	...
	while (offset < NPROCS) {
		if (proc_array[offset].p_state == P_BLOCKED && 
		    proc_array[offset].next == current->p_pid) {
			reaped = 1;
...
//find an empty process descriptor (not including proc_array[0])
	int x; 
	process_t* child; 
	for (x = 1; x < NPROCS; x++) {
		child = &proc_array[x];
		if (child->p_state == P_EMPTY) {
			//if we find empty process, copy parent's reg + stack into child 
			child->p_registers = parent->p_registers; //set registers from parent to that of child 
			child->p_registers.reg_eax = 0; //return 0 to the reg eax 
			child->p_state = P_RUNNABLE; //make child runnable 
			copy_stack(child, parent);     //dest is the child, src is the parent 
			return x; //return child's process ID to the parent 
		}
	}
	//if we are here, then we've looped through every process and couldn't find an empty process 
...
copy_stack(process_t *dest, process_t *src)
{
	...
	int src_pid = src->p_pid;
	int dest_pid = dest->p_pid;
	...
	size_t dest_size = dest_stack_top - dest_stack_bottom;
	// YOUR CODE HERE: memcpy the stack and set dest->p_registers.reg_esp
	memcpy((void*)dest_stack_top, (void*)src_stack_top, dest_size);
	dest->p_registers.reg_esp = dest_stack_bottom; //just like in the example, reg_esp = the bottom of the stack
}
